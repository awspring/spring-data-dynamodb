...
| 70 | `ItemCollectionUseCase` | Base-table and index item collections, fixed finders, explicit key conditions, named key conditions, and query limits |
| 80 | `PaginationUseCase` | Repository `Window<T>` keyset scrolling and template `DynamoDbPageRequest`/`LastEvaluatedKey` paging |
| 90 | `InnerClassUseCase` | Nested-map values and flattened sort-key-routed `@InnerClass` rows |
...
metadata supplies aliases and per-request limits for both forms.
