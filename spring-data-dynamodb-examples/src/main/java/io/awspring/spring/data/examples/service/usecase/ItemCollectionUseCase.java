...
import io.awspring.spring.data.examples.model.OrderItemCollectionRecord;
import io.awspring.spring.data.examples.repository.OrderItemCollectionByIndexRepository;
import io.awspring.spring.data.examples.repository.OrderItemCollectionRecordRepository;
...
public class ItemCollectionUseCase implements ExampleUseCase {
	...
	private final OrderItemCollectionRecordRepository recordViews;

	public ItemCollectionUseCase(OrderItemCollectionRepository tableViews,
			OrderItemCollectionByIndexRepository indexViews, OrderItemCollectionRecordRepository recordViews) {
	...
	public void run() {
		...
		Optional<OrderItemCollectionRecord> record = recordViews.findByPartitionKey(ExampleData.CUSTOMER_PK);
		System.out.println("fixed=" + fixed.isPresent() + ", explicit=" + explicit.isPresent() + ", named="
				+ named.isPresent() + ", index=" + indexed.isPresent() + ", record="
				+ record.map(OrderItemCollectionRecord::order).isPresent());
	}
}
