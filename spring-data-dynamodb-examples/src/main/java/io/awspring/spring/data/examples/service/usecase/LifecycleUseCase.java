...
@Component
@Order(110)
public class LifecycleUseCase
		implements ExampleUseCase, DynamoDbBeforeConvertCallback<Customer>, DynamoDbAfterSaveCallback<Customer> {

	private final CustomerRepository customers;
	private final AtomicInteger callbacks = new AtomicInteger();
	private final AtomicInteger events = new AtomicInteger();

	public LifecycleUseCase(CustomerRepository customers) {
	...
	public void run() {
		...
		System.out.println("customer callback invocations=" + callbacks.get() + ", after-save events=" + events.get());
	}

	@Override
	public Customer onBeforeConvert(Customer entity, String tableName) {
		callbacks.incrementAndGet();
		return entity;
	}

	@Override
	public Customer onAfterSave(Customer entity, String tableName) {
		callbacks.incrementAndGet();
		return entity;
	}

	@EventListener
	void onAfterSave(DynamoDbAfterSaveEvent<?> event) {
