...
class KeyTemplateResolverTest {
	...
	@Table(tableName = "repeatable_templates")
	@SortKeyTemplate(value = "ROUND#{round}", column = "gsi1sk")
	@SortKeyTemplate("MATCH#{year}#{round}")
	static class MatchWithRepeatableTemplates {
		@PartitionKey
		String tournamentId;
		int year;
		String round;
	...
	@Nested
	@DisplayName("Repeatable templates")
	class RepeatableTemplates {

		@Test
		@DisplayName("Preserves annotation declaration order")
		void preservesAnnotationDeclarationOrder() {
			DynamoDbPersistentEntity<?> entity = entityFor(MatchWithRepeatableTemplates.class);
			KeyTemplateResolver resolver = KeyTemplateResolver.forEntity(entity);

			assertEquals(List.of(COLUMN_GSI1SK, COLUMN_SK), List.copyOf(resolver.templateIndexes()));
		}
	}
