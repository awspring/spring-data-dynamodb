...
public class CachingDynamoDbPersistentProperty extends BasicDynamoDbPersistentProperty {
	...
	public CachingDynamoDbPersistentProperty(Property property, DynamoDbPersistentEntity<?> owner,
		...
		if (isEmbedded) {
			...
			else if (property.getField().get().isAnnotationPresent(Embedded.class)) {
				Embedded embedded = property.getField().get().getAnnotation(Embedded.class);
				startsWith = embedded.startsWith();
				endsWith = embedded.endsWith();
				regexPattern = compile(embedded.regex(), property);
				serializeAsNestedMap = embedded.serializeAsNestedMap();
	...
	private static Pattern compile(String regex, Property property) {
		...
		try {
			...
			throw new MappingException(
					"@Embedded(regex = \"" + regex + "\") on " + property + " is not a valid regular expression", e);
		}
	}
