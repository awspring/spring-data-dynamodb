...
public final class KeyTemplateResolver {
	...
	private static final String BASE_TABLE_SORT_KEY_COLUMN = "sk";
	...
	public static KeyTemplateResolver forEntity(DynamoDbPersistentEntity<?> entity) {
		...
		for (SortKeyTemplate annotation : AnnotatedElementUtils.findMergedRepeatableAnnotations(entity.getType(),
			...
			String column = StringUtils.hasText(annotation.column()) ? annotation.column() : BASE_TABLE_SORT_KEY_COLUMN;
		...
		return new KeyTemplateResolver(entity, Collections.unmodifiableMap(new LinkedHashMap<>(templates)),
	...
	@Nullable
	public String baseTableSortKeyColumn() {
		return templatesByIndex.containsKey(BASE_TABLE_SORT_KEY_COLUMN) ? BASE_TABLE_SORT_KEY_COLUMN : null;
	}
