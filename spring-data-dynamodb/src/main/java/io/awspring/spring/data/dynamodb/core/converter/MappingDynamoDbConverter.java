...
public class MappingDynamoDbConverter extends AbstractDynamoDbConverter
	...
	public void writeKey(Object partitionKey, @Nullable Object sortKey, Map<String, AttributeValue> keys,
		...
		if (sortKey != null) {
			...
			String templateColumn = KeyTemplateResolver.forEntity(persistenceEntity).baseTableSortKeyColumn();
			Assert.notNull(templateColumn,
					"Entity has no base-table sort key or @SortKeyTemplate to bind a sortKey argument to");
			keys.put(templateColumn, toAttributeValue(sortKey, false));
		}
	}
	...
	public void updateRecursive(Object objectToUpdate, Map<String, AttributeValueUpdate> values,
		...
		for (DynamoDbPersistentProperty property : entity) {
			if (property.isIdProperty() || isBaseTableSortKey(property, entity) || property.isDerived()) {
				continue;
			}
			if (propertyValueConversions.hasValueConverter(property)) {
	...
	private void writeSortKeyTemplateUpdates(Object obj, Map<String, AttributeValueUpdate> values,
		...
		String baseTableSortKeyColumn = resolver.baseTableSortKeyColumn();
		for (String column : resolver.templateIndexes()) {
			if (column.equals(baseTableSortKeyColumn)) {
				continue;
			}
			String composed = resolver.compose(column, obj, getConversionService());
	...
	private boolean isBaseTableSortKey(DynamoDbPersistentProperty property, DynamoDbPersistentEntity<?> entity) {
		String templateColumn = KeyTemplateResolver.forEntity(entity).baseTableSortKeyColumn();
		if (property.getColumnName().equals(templateColumn)) {
			return true;
		}
	...
	@Nullable
	private Object readEmbedded(DynamoDbPersistentProperty property, Map<String, AttributeValue> source,
	...
	public <R> R read(Map<String, AttributeValue> source, DynamoDbPersistentEntity<R> entity) {
		...
		for (DynamoDbPersistentProperty property : entity) {
			...
			if (propertyValueConversions.hasValueConverter(property)) {
				...
				propertyAccessor.setProperty(property, readEmbedded(property, source, entity));
			}
			else {
	...
	public <A> A readItemCollection(List<Map<String, AttributeValue>> items, DynamoDbPersistentEntity<A> viewEntity) {
		...
		Map<DynamoDbPersistentProperty, Object> members = new LinkedHashMap<>();
		for (DynamoDbPersistentProperty property : viewEntity) {
			if (!property.isItemCollectionMember()) {
				continue;
			}
			Object value = property.isCollectionLike() ? readItemCollectionMemberList(items, sortKeyColumn, property)
					: readItemCollectionMemberSingle(items, sortKeyColumn, property, viewEntity);
			members.put(property, value);
		}

		InstanceCreatorMetadata<DynamoDbPersistentProperty> creatorMetadata = viewEntity.getInstanceCreatorMetadata();

		if (creatorMetadata != null && creatorMetadata.hasParameters()) {
			return instantiators.getInstantiatorFor(viewEntity).createInstance(viewEntity,
					new RecordMemberParameterValueProvider(viewEntity, members));
		}
		...
		for (Map.Entry<DynamoDbPersistentProperty, Object> entry : members.entrySet()) {
			accessor.setProperty(entry.getKey(), entry.getValue());
		}

		return instance;
	}

	private static final class RecordMemberParameterValueProvider
			implements ParameterValueProvider<DynamoDbPersistentProperty> {

		private final DynamoDbPersistentEntity<?> viewEntity;
		private final Map<DynamoDbPersistentProperty, Object> recordMembers;

		private RecordMemberParameterValueProvider(DynamoDbPersistentEntity<?> viewEntity,
				Map<DynamoDbPersistentProperty, Object> recordMembers) {
		...
		@Override
		@SuppressWarnings("unchecked")
		public <T> @Nullable T getParameterValue(Parameter<T, DynamoDbPersistentProperty> parameter) {
			String name = parameter.getName();
			if (name == null) {
				return null;
			}
			DynamoDbPersistentProperty property = viewEntity.getPersistentProperty(name);
			return property != null ? (T) recordMembers.get(property) : null;
		}
	}
	...
	private Object resolvePropertyValue(DynamoDbPersistentProperty property, Map<String, AttributeValue> source,
		...
		if (property.isSpecialType() && !property.serializeAsNestedMap()) {
			return readEmbedded(property, source, owner);
		}
		Class<?> elementType = null;
		if (property.isCollectionLike() && property.getTypeInformation().getComponentType() != null) {
			elementType = property.getTypeInformation().getComponentType().getType();
	...
	private void fetchKeysAndPopulate(Object toBeUsed, Map<String, AttributeValue> keys,
		...
		if (templateColumn != null) {
			String composed = resolver.compose(templateColumn, toBeUsed, getConversionService());
			Assert.notNull(composed, "@SortKeyTemplate did not compose a base-table sort key");
			keys.put(templateColumn, AttributeValue.builder().s(composed).build());
		}
	}
		...
		for (Field field : fields) {
			...
			try {
				...
				throw new MappingException("Cannot read field '" + field.getName() + "' of " + value.getClass()
						+ " for @Embedded(serializeAsNestedMap = true) encoding", e);
			}
		}
	...
	private Object fromNativeStructure(Map<String, AttributeValue> source, Class<?> type) {
		try {
			...
			throw new MappingException("Cannot reconstruct " + type
					+ " from its nested-map encoding (@Embedded(serializeAsNestedMap = true))"
